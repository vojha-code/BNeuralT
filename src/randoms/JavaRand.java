...
public class JavaRand extends java.util.Random {
	...
	/**
	 * Return random integer from Poison with parameter lambda. The mean of
	...
	public double random(double low, double high, boolean isAuto) {
		if (isAuto) {
			return Math.random() * (high - low) + low;//take from system
		} else {
			return random(false) * (high - low) + low;//take from seed
		}
	}//random
