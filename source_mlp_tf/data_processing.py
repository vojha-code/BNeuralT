...
class DataProcessing:
    ...
    def load_data(self, p_problem_type, dataset = 'input_a_file.csv'):
        ...
        if path.exists(fromFile):
            print('DATA: loading file ', dataset)
    ...
    def holdout_method(self, val_set = False, training_set = 0.8):
        ...
        print('    training set   :', len(data_inputs[0]),' -> ', round(len(data_inputs[0])*100.00/ len(data_input_values)), '%')
        print('    test set       :', len(data_inputs[1]),' -> ',round(len(data_inputs[1])*100.00/ len(data_input_values)), '%')
        if val_set:
            print('    validation set :', round(len(data_input_values[2])*100.00/ len(data_input_values)), ' %')
